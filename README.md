...
### 보너스
    ...
    - /shorten api 호출 시 테스트 진행을 위해 5초 뒤에 만료되는 단축 URL을 생성합니다. 테스트 코드 실행 시 만료를 처리하기 위해 셀러리에 특정 task(update_urls)를 등록합니다. 해당 task가 호출 되는 기준 시간보다 전에 있을 경우 is_active = False를 주어 만료처리 시킵니다.
    ...
    # 테스트 코드
    테스트 코드는 크게 3가지로 분류하여 tests 디렉토리 내에 작성하였습니다. 1. API 호출과 관련한 테스트 코드. 2. 해당 API 호출 시 내부에 사용되는 함수와 관련한 테스트 코드. 3. Celery task 관련 테스트코드 입니다.
